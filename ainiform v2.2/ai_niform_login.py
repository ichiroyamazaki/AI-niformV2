...
class StudentTeacherSplashScreen:
    ...
    def create_profile_section(self, parent):
        ...
        profile_image = self.load_profile_image()
        if profile_image:
        ...
        # Name and role
        name_label = tk.Label(profile_frame, text=self.person_data['name'], 
                             font=("Arial", 18, "bold"), bg='#4A90E2', fg='white')
        name_label.pack(pady=(10, 5))
        
        role_label = tk.Label(profile_frame, text=f"({self.person_data['role']})", 
...
class AINiformLogin:
    ...
    def start_splash_camera_feed(self):
        ...
        self.update_splash_camera_feed()
            ...
            if frame is not None:
                ...
                if self.splash_camera_detector.model is not None:
                    ...
                    if detections:
                        ...
                        uniform_detected = any('uniform' in det['class_name'].lower() for det in detections)
                        if uniform_detected:
                            # Uniform detected - clean
                            self.compliance_result = "clean"
                        else:
                            # No uniform detected - violation
                            self.compliance_result = "violation"
                    else:
                        # No detections - clean
                        self.compliance_result = "clean"
                else:
