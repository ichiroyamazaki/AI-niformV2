...
class ColoredButton(tk.Canvas):
    """Custom colored button using Canvas for better macOS compatibility"""
    def __init__(self, parent, text, command=None, bg='#007BFF', fg='white', 
                 active_bg=None, font=('Arial', 12), width=None, height=None, **kwargs):
        if active_bg is None:
            # Create a darker version of bg for active state
            active_bg = self._darken_color(bg)
        
        if width is None:
            width = len(text) * 10 + 40
        if height is None:
            height = 35
    ...
    def _draw_button(self, bg=None):
        """Draw the button with the specified background"""
        if bg is None:
            bg = self.bg
...
class MacOSButton(tk.Button):
    """Custom button class that forces colors on macOS"""
    def __init__(self, parent, **kwargs):
        super().__init__(parent, **kwargs)
        self._force_colors()
    
    def _force_colors(self):
        """Force button colors on macOS"""
        if platform.system() == "Darwin":  # macOS
            try:
                # Force the button to use our colors
                self.configure(relief='flat', borderwidth=0, highlightthickness=0)
                # Additional macOS-specific overrides
                self.configure(overrelief='flat')
                
                # More aggressive macOS color forcing
                self.configure(selectcolor='black')  # Force selection color
                
                # Bind events to maintain colors
                self.bind('<Enter>', self._on_enter)
                self.bind('<Leave>', self._on_leave)
                self.bind('<Button-1>', self._on_click)
                self.bind('<ButtonRelease-1>', self._on_release)
    ...
    def _on_enter(self, event):
        """Maintain colors on mouse enter"""
    ...
    def _on_leave(self, event):
        """Maintain colors on mouse leave"""
        if platform.system() == "Darwin":
            try:
                self.configure(relief='flat', borderwidth=0)
            except:
...
class AINiformApp:
    ...
    def show_login_screen(self):
        ...
        # Log-in button
        login_button = ColoredButton(
            content_area,
            text="Log-in",
            bg='#28a745',  # Green
            fg='white',
            active_bg='#218838',  # Darker green for hover
            font=('Arial', 14, 'bold'),
            width=120,
            height=45,
    ...
    def create_top_banner(self):
        """Create the red top banner with 'Turnstile is Closed' text - only on left side"""
        # Create the banner container
        banner_container = tk.Frame(self.content_frame, height=80)
        banner_container.pack(fill='x')
        banner_container.pack_propagate(False)
        
        # Red banner on the left side only
        red_banner = tk.Frame(banner_container, bg='#EA234C', height=80, width=500)
        red_banner.pack(side='left', fill='y')
        red_banner.pack_propagate(False)
        
        # Turnstile status text in red banner
        ...
        # Blue section on the right side of the banner
        blue_banner = tk.Frame(banner_container, bg='#1E90FF', height=80)
        blue_banner.pack(side='right', fill='both', expand=True)
        blue_banner.pack_propagate(False)
        
        # STI logo in the top-right corner of the blue banner
        try:
            logo_path = os.path.join("image-elements", "STI Balagtas Logo.png")
            if os.path.exists(logo_path):
                # Load and resize the logo
                logo_image = Image.open(logo_path)
                # Small logo for the banner
                logo_width = 50
                logo_height = int(logo_width * logo_image.height / logo_image.width)
                logo_image = logo_image.resize((logo_width, logo_height), Image.Resampling.LANCZOS)
                logo_photo = ImageTk.PhotoImage(logo_image)
                
                # Create logo label in top-right corner
                logo_label = tk.Label(blue_banner, image=logo_photo, bg='#1E90FF')
                ...
                sti_label.pack(anchor='ne', padx=10, pady=5)
    ...
    def create_main_content(self):
        ...
        content_area = tk.Frame(self.content_frame, bg='white')
        content_area.pack(fill='both', expand=True, pady=(0, 0))
        ...
        # Right side (blue panel with instructions) - extend to top
        right_panel = tk.Frame(content_area, bg='#1E90FF', width=400)
        right_panel.pack(side='right', fill='y')
        right_panel.pack_propagate(False)
        
        # Instructions text - centered in the panel
        instructions_frame = tk.Frame(right_panel, bg='#1E90FF')
        instructions_frame.pack(expand=True, padx=40, pady=20)
...
Reader."""
        ...
        # Card input section - positioned at bottom of right panel
        card_frame = tk.Frame(right_panel, bg='#1E90FF')
        card_frame.pack(side='bottom', fill='x', padx=40, pady=20)
        ...
        guard_id_label.pack(anchor='w', pady=(0, 5))
        ...
        self.card_entry.pack(anchor='w', pady=(0, 10))
    ...
    def update_time(self):
        ...
        try:
            if self.time_label and self.date_label and self.time_label.winfo_exists() and self.date_label.winfo_exists():
                now = datetime.now()
            ...
            # Silently handle any errors with time updates
            pass
    ...
    def create_guard_main_content(self):
        ...
        # Visitor button
        visitor_button = ColoredButton(
            content_frame,
            text="Visitor",
            bg='#FFD700',  # Yellow
            fg='black',
            active_bg='#FFC107',  # Darker yellow for hover
            font=('Arial', 18, 'bold'),
            width=320,
            height=50,
    ...
    def create_visitor_form_content(self, content_area):
        ...
        # Submit button
        submit_button = ColoredButton(
            form_frame,
            text="Submit",
            bg='#007BFF',  # Blue
            fg='white',
            active_bg='#0056b3',  # Darker blue for hover
            font=('Arial', 14, 'bold'),
            width=100,
            height=40,
        ...
        # Visitor button (bright yellow) - using ColoredButton
        visitor_button = ColoredButton(
            content_frame,
            text="Visitor",
        ...
        # Student button (light grey) - using ColoredButton
        student_button = ColoredButton(
            content_frame,
            text="Student",
            bg='#d3d3d3',  # Light grey
            fg='black',
            active_bg='#c0c0c0',  # Darker grey for hover
            font=('Arial', 18, 'bold'),
            width=320,
            height=50,
            command=self.student_action
